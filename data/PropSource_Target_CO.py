...
class PropSource_Target_CO:
  ...
  Subset random: It randomly chooses, without repetition, the samples from each subset.
    ...
    ##repeat the simulation n times varying nY, nCB, nUR and nCF
        ...
        #choose which reduction to print figure ### in this script plot2D=nsample
            ...
            # Save the data to a csv file
                    ...
                    if idx == 1: 
                        ...
                        pd.concat(raw_data).to_csv(f"Simulation_CO_Ps_Run{n}_CB_Target{ysamp}.csv",index=False)
                        pd.concat(raw_area_data).to_csv(f"Simulation_CO_Area_Run{n}_CB_AllTarget.csv", index=False) 
                    if idx == 2:
                        pd.concat(raw_data).to_csv(f"Simulation_CO_Ps_Run{n}_UR_Target{ysamp}.csv", index=False)
                        pd.concat(raw_area_data).to_csv(f"Simulation_CO_Area_Run{n}_UR_AllTarget.csv", index=False) 
                    if idx == 3:
                        pd.concat(raw_data).to_csv(f"Simulation_CO_Ps_Run{n}_CF_Target{ysamp}.csv", index=False)
                        pd.concat(raw_area_data).to_csv(f"Simulation_CO_Area_Run{n}_CF_AllTarget.csv", index=False) 
        ...
        if idx == 1:
                raw_CV_df.to_csv(f"Resume_CV2_Run{n}_CB_CO.csv", index=False) 
                raw_Summ_df.to_csv(f"Resume_Solve2_Run{n}_CB_CO.csv", index=False) 
        if idx == 2:
                raw_CV_df.to_csv(f"Resume_CV2_Run{n}_UR_CO.csv", index=False)
                raw_Summ_df.to_csv(f"Resume_Solve2_Run{n}_UR_CO.csv", index=False) 
        if idx == 3:
                raw_CV_df.to_csv(f"Resume_CV2_Run{n}_CF_CO.csv", index=False)
                raw_Summ_df.to_csv(f"Resume_Solve2_Run{n}_CF_CO.csv", index=False) 
